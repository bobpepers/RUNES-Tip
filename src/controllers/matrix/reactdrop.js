...
export const listenMatrixReactDrop = async (
  ...
  // const reactDropUserId = reactDrop.user.user_id.replace('matrix-', '');
  ...
  const listenerFunction = async (
    ...
    if (
      ...
      if (findReactUser) {
        const reactDropTipUserId = findReactUser.user_id.replace('matrix-', '');
        ...
        if (!findReactTip) {
          ...
          if (reactDrop.emoji !== confirmMessage.event.content['m.relates_to'].key) {
            ...
            const listenerFunctionForUser = async (
              ...
              if (
                confirmUserMessage.sender.userId === reactDropTipUserId
                && room.roomId === userDirectMessageRoomId
              ) {
