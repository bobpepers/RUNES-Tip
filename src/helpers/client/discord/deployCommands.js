...
const commands = [
  new SlashCommandBuilder().setName(`${settings.bot.command.discord.slash}`).setDescription(`'Use ${settings.bot.name}`)
    .addSubcommand(
    ...
    .addSubcommand(
      (subcommand) => subcommand
        ...
        .addStringOption((option) => option.setName('time').setDescription('Choose a time (examples: 60s, 2m, 1h, 1d) (min: 60s) (max: 2d)').setRequired(true))
        .addStringOption((option) => option.setName('emoji').setDescription('Choose an emoji to use').setRequired(true)),
    )
    .addSubcommand(
      (subcommand) => subcommand
        .setName('flood')
        ...
        .addStringOption((option) => option.setName('amount').setDescription('Enter an amount').setRequired(true))
        .addStringOption((option) => option.setName('role').setDescription('Optional role')),
    )
    .addSubcommand(
      (subcommand) => subcommand
        .setName('rain')
