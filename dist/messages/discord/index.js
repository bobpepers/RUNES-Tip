...
var tipFaucetSuccessMessage = function tipFaucetSuccessMessage(message, amount) {
  var result = new _discord.MessageEmbed().setColor(settings.bot.color).setTitle('Tip').setDescription("<@".concat(message.author.id, "> tipped ").concat(amount / 1e8, " ").concat(settings.coin.ticker, " to Faucet")).setTimestamp().setFooter("".concat(settings.bot.name, " v").concat(_package["default"].version), settings.coin.logo);
  return result;
};
