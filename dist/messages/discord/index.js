...
var maxTimeReactdropMessage = function maxTimeReactdropMessage(message) {
  var result = new _discord.MessageEmbed().setColor(settings.bot.color).setTitle('Reactdrop').setDescription("Maxiumum time is 2 weeks").setTimestamp().setFooter("".concat(settings.bot.name, " v").concat(_package["default"].version), settings.coin.logo);
  return result;
};
